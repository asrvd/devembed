(function () {
  const url =
    window.location.hostname === "localhost"
      ? "http://localhost:3000"
      : "https://devembed.asrvd.me";
  const payload = document.querySelector("script[data-username]");
  //@ts-ignore
  const { username, placement } = payload.dataset;
  if (!username) return;
  //@ts-ignore
  const attr = payload.getAttribute.bind(payload);
  const searchParams = attr("data-username");
  ...
  containerButton.append(constructIframe(`${url}/widget/panel/trigger`));
